...
// Testfarm is the chef of new tokens. He can make yummy food and he is a fair guy as well as MasterChef.
contract TestFarm {
    using SafeMath for uint256;
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many TFT tokens the user has provided.
    ...
    // Info of each user that stakes tft tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = tft.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 tftSupply = tft.balanceOf(address(this));
        if (tftSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(tftSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit tft tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        tft.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw tft tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        tft.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        tft.safeTransfer(address(msg.sender), user.amount);
